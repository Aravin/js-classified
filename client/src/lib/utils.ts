 ...
 */
export async function checkActiveAdsLimit(
  userId: string,
  excludeListingId?: number,
): Promise<{ hasReachedLimit: boolean; activeCount: number; activeLimit: number }> {
  try {
    ...
    const activeCount = listings.filter(
      (listing: { status: string; createdAt: string; republishedAt?: string | null; id: number }) =>
    ...
    const activeLimit =
      userLimit !== undefined && userLimit !== null ? userLimit : config.user.maxActiveAds;
